...
- (NSImage*)head:(UInt32)myQQ handleStatus:(BOOL)handleStatus {
	...
	if([self hasCustomHead] && m_customHead != nil) {
		NSString* path = [FileTool getCustomHeadPath:myQQ md5:[m_customHead md5]];
		imgMain = [NSImage imageNamed:path];
		if(imgMain == nil) {
			if([FileTool isFileExist:path]) {
				imgMain = [[[NSImage alloc] initWithContentsOfFile:path] autorelease];
				[imgMain setName:path];
			}
		}
	}
	...
	if(imgMain == nil) {
		if([self isTM]) {
			NSString* name = [self isMM] ? @"tm_female.gif" : @"tm_male.gif";
			imgMain = [NSImage imageNamed:name];
		} else
...
- (id)initWithQQ:(UInt32)QQ domain:(MainWindowController*)domain {
	...
	if(self) {
		...
		m_status = kQQStatusOffline;
		m_messageCount = 0;
		m_mobileMessageCount = 0;
		m_tempSessionMessageCount = 0;
		m_onlining = NO;
...
- (void)increaseTempSessionMessageCount {
	m_tempSessionMessageCount++;
	
	[[NSNotificationCenter defaultCenter] postNotificationName:kModelMessageCountChangedNotificationName
														object:self
													  userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:(m_tempSessionMessageCount - 1)], kUserInfoOldMessageCount, [NSNumber numberWithInt:m_tempSessionMessageCount], kUserInfoNewMessageCount, m_domain, kUserInfoDomain, nil]];
}
...
- (void)increaseMobileMessageCount {
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:kModelMessageCountChangedNotificationName
														object:self
													  userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:(m_mobileMessageCount - 1)], kUserInfoOldMessageCount, [NSNumber numberWithInt:m_mobileMessageCount], kUserInfoNewMessageCount, m_domain, kUserInfoDomain, nil]];
}
...
- (UInt32)tempSessionMessageCount {
	return m_tempSessionMessageCount;
}

- (void)setTempSessionMessageCount:(UInt32)count {
	NSNumber* oldCount = [NSNumber numberWithInt:m_tempSessionMessageCount];
	m_tempSessionMessageCount = count;
...
- (UInt32)mobileMessageCount {
	return m_mobileMessageCount;
}

- (void)setMobileMessageCount:(UInt32)count {
	NSNumber* oldCount = [NSNumber numberWithInt:m_mobileMessageCount];
	m_mobileMessageCount = count;

...
- (NSRect) closeButtonRectForTabCell:(PSMTabBarCell *)cell
{
    ...
    if([cell state] == NSOnState){
        result.origin.y -= [cell isUpHead] ? -1 : 1;
    }
...
- (void)drawTabCell:(PSMTabBarCell *)cell
{
    ...
    if ([cell state] == NSOnState) {
		if([cell isUpHead]) {
			// selected tab
			NSRect aRect = NSMakeRect(cellFrame.origin.x, cellFrame.origin.y + 2.5, cellFrame.size.width, cellFrame.size.height-2.5);
			aRect.origin.y += 0.5;
			...
			[bezier lineToPoint:NSMakePoint(aRect.origin.x, aRect.origin.y + 1.5)];
			[bezier lineToPoint:NSMakePoint(aRect.origin.x + 1.5, aRect.origin.y)];
			[bezier lineToPoint:NSMakePoint(aRect.origin.x + aRect.size.width - 1.5, aRect.origin.y)];
			[bezier lineToPoint:NSMakePoint(aRect.origin.x + aRect.size.width, aRect.origin.y + 1.5)];
...
- (void)drawInteriorWithTabCell:(PSMTabBarCell *)cell inView:(NSView*)controlView
{
    ...
    if([cell count] > 0){
        ...
        if([cell state] == NSOnState)
            myRect.origin.y -= [cell isUpHead] ? -1.0 : 1.0;
	...
	if(![cell isLabelSuppressed]) {
		...
		if([cell state] == NSOnState){
			labelRect.origin.y -= [cell isUpHead] ? -1 : 1;
		}

...
- (BOOL)showCloseTabButton {
	return _showCloseTabButton;
}

- (void)setShowCloseTabButton:(BOOL)value {
	_showCloseTabButton = value;
	[self update];
}
...
- (void)closeTab:(NSTabViewItem*)item {
	NSEnumerator* e = [_cells objectEnumerator];
	PSMTabBarCell* cell = [e nextObject];
	while(cell != nil) {
...
- (void)addTabViewItem:(NSTabViewItem *)item
{
	// delegate
	if(delegate)
		[delegate tabBar:self willAddTabForItem:item];
	...
	// set show clost tab button
	[cell setHasCloseButton:_showCloseTabButton];
	[cell setUpHead:_isUpHead];
...
- (void)update
{
    ...
    for(i = 0; i < cellCount; i++){
		...
		[cell setHasCloseButton:_showCloseTabButton];
		[cell setUpHead:_isUpHead];
		[cell setLabelSuppressed:!_showLabel];
        
        // Determine cell width
        if(_sizeCellsToFit){
            width = [cell desiredWidthOfCell];
			width = MIN(_cellMaxWidth, width);
			width = MAX(_cellMinWidth, width);
        } else {
...
- (void)encodeWithCoder:(NSCoder *)aCoder 
{
    ...
    if ([aCoder allowsKeyedCoding]) {
		...
		[aCoder encodeBool:_showCloseTabButton forKey:@"PSMShowCloseTabButton"];
		[aCoder encodeBool:_isUpHead forKey:@"PSMUpHead"];
...
- (id)initWithCoder:(NSCoder *)aDecoder 
{
    ...
    if (self) {
        if ([aDecoder allowsKeyedCoding]) {
			...
			_showCloseTabButton = [aDecoder decodeBoolForKey:@"PSMShowCloseTabButton"];
			_isUpHead = [aDecoder decodeBoolForKey:@"PSMUpHead"];

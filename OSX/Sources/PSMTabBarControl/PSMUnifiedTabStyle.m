...
- (NSRect)iconRectForTabCell:(PSMTabBarCell *)cell
{
	...
	if(![cell isLabelSuppressed] && [[cell stringValue] length] != 0 || [cell count] != 0) {		
		result.origin.x = cellFrame.origin.x + MARGIN_X;
		...
		result.origin.x = cellFrame.origin.x + (NSWidth(cellFrame) - kPSMTabBarIconWidth) / 2;
		result.origin.y = cellFrame.origin.y + MARGIN_Y;
	}
...
- (void)drawTabCell:(PSMTabBarCell *)cell
{
    ...
    if ([cell state] == NSOnState)
	{
		if([cell isUpHead]) {
			// selected tab
			NSRect aRect = NSMakeRect(cellFrame.origin.x + 0.5, cellFrame.origin.y + 0.5, cellFrame.size.width - 1.0, cellFrame.size.height - 0.5);
			...
			[bezier appendBezierPathWithArcWithCenter:NSMakePoint(NSMinX(rect), NSMaxY(rect)) radius:radius startAngle:90 endAngle:180.0];
...
- (void)drawInteriorWithTabCell:(PSMTabBarCell *)cell inView:(NSView*)controlView
{
    ...
    // label rect
	if(![cell isLabelSuppressed] && [[cell stringValue] length] != 0) {
		NSRect labelRect;

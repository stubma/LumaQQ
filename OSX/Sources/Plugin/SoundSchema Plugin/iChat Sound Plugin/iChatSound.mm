...
- (NSString*)pluginDescription {
	return NSLocalizedStringFromTableInBundle(@"LQPluginDescription", @"iChatSound", [NSBundle bundleForClass:[self class]], nil);
}
...
- (NSString*)soundPath:(int)soundId {
	...
	switch(soundId) {
		...
		case kSoundIdLogin:
			return [bundle pathForResource:@"login" ofType:@"aif"];
		case kSoundIdLogout:
			return [bundle pathForResource:@"logout" ofType:@"aif"];
		case kSoundIdKickedOut:
			return [bundle pathForResource:@"kicked_out" ofType:@"aif"];
		case kSoundIdMessageBlocked:
			return [bundle pathForResource:@"message_blocked" ofType:@"aif"];
		default:
			return @"";
	}
}

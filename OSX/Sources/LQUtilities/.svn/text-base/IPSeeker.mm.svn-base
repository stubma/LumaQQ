...
- (NSString*)getLocation:(const char*)ip {
	return [self getLocation:ip locationOnly:NO];
}
...
- (NSString*)getLocation:(const char*)ip locationOnly:(BOOL)locationOnly {
	...
	NSString* ipStr = [ByteTool ip2String:ip];
	...
	if(locationOnly)
		...
		return [NSString stringWithFormat:L(@"LQIPLocation"), [ByteTool ip2String:ip], loc];
}
...
- (NSString*)readString:(unsigned long long)offset {
	...
	return [ByteTool getString:data];
}

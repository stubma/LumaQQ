...
@implementation ClusterModifyMessageSettingPacket

- (id)initWithQQUser:(QQUser*)user {
	self = [super initWithQQUser:user];
	if(self) {
		m_subCommand = kQQSubCommandClusterModifyMessageSetting;
	}
	return self;
}
...
- (void)fillBody:(ByteBuffer*)buf {
	[super fillBody:buf];
	[buf writeUInt32:m_externalId];
	[buf writeByte:m_messageSetting];
}

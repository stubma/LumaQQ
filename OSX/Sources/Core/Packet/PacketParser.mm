...
- (id)initWithKeyProvider:(id<KeyProvider>)provider {
	self = [super init];
	if(self) {
		m_keyProvider = [(id)provider retain];
	}
	return self;
}

- (void) dealloc {
	[(id)m_keyProvider release];
	[super dealloc];
}
...
- (InPacket*)packetWithData:(NSData*)data user:(QQUser*)user {
	...
	} else if(bytes[0] == kQQHeaderAgentFamily) {
		UInt16 command = [ByteTool getUInt16:bytes offset:5];
		switch(command) {
			case kQQCommandRequestAgent:
				packet = [[RequestAgentReplyPacket alloc] initWithData:data user:user];
				break;
			case kQQCommandRequestBegin:
				packet = [[RequestBeginReplyPacket alloc] initWithData:data 
																  user:user 
															decryptKey:[m_keyProvider clusterCustomFaceAgentKey]];
				break;
			case kQQCommandRequestFace:
				packet = [[RequestFaceReplyPacket alloc] initWithData:data 
																 user:user 
														   decryptKey:[m_keyProvider clusterCustomFaceAgentKey]];
				NSLog(@"request face reply hash: %d", [packet hash]);
				break;
			case kQQCommandTransfer:
				packet = [[ServerTransferPacket alloc] initWithData:data user:user];
				break;
		}

...
- (BOOL)isDebugging {
	return [m_debugListeners count] > 0;
}

- (void)addDebugListener:(id<DebugListener>)listener {
	[m_debugListeners addObject:listener];
}

- (void)removeDebugListener:(id<DebugListener>)listener {
	[m_debugListeners removeObject:listener];
}

- (void)deliveryDebugEvent:(DebugEvent*)event {
	NSEnumerator* e = [m_debugListeners objectEnumerator];
	while(id<DebugListener> listener = [e nextObject])
